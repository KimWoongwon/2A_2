...
void Matrix_Function::ScalarMultiple(Matrix3x3& _m, const int _scalar)
{
	for (int i = 0; i < Matrix3x3::SIZE; i++)
		for (int j = 0; j < Matrix3x3::SIZE; j++)
...
Matrix3x3 Multiple(const Matrix3x1& _vm, const Matrix1x3& _hm)
{
...
float Multiple(const Matrix1x3& _hm, const Matrix3x1& _vm)
{
...
{
	Vector2D temp;
	temp.x = (_v.x * _cm._11) + (_v.y * _cm._21) + (_v.w * _cm._31);
	temp.y = (_v.x * _cm._12) + (_v.y * _cm._22) + (_v.w * _cm._32);
	temp.w = (_v.x * _cm._13) + (_v.y * _cm._23) + (_v.w * _cm._33);
	_v = temp;
}
...
void Matrix_Function::Rotate_2D(Matrix3x3& _m, const float _radian)
{
	_m._11 = cosf(_radian);
	_m._12 = sinf(_radian);
	_m._21 = -sinf(_radian);

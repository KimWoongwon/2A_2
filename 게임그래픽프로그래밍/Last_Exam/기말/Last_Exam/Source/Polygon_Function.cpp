#include "../Header/Polygon_Function.h"

void Polygon_Function::Move(Triangle2D& _tri, const Point& _moveval)
{
	for (int i = 0; i < Triangle2D::Vertex_Count; i++)
	{
		_tri.vertexs[i].vector.x = (_tri.vertexs[i].vector.x + _moveval.x);
		_tri.vertexs[i].vector.y = (_tri.vertexs[i].vector.y + _moveval.y);
	}
	_tri.setCenter();
}
...
void Polygon_Function::Translate(Triangle2D& _tri, const Point& _moveval)
{
	...
	for (int i = 0; i < Triangle2D::Vertex_Count; i++)
	{
		Vector3D temp(_tri.vertexs[i].vector.x, _tri.vertexs[i].vector.y, 1, 1);
		Matrix_Function::Vector3D_Multiple(temp, Trans_value);
		_tri.vertexs[i].vector.setPos(temp.x, temp.y);
	}
	_tri.setCenter();
}
...
void Polygon_Function::Rotate(Triangle2D& _tri, const float _angle)
{
	...
	for (int i = 0; i < Triangle2D::Vertex_Count; i++)
	{
		...
		_tri.vertexs[i].setPos(temp.x, temp.y);
	}
	_tri.setCenter();
}

...
DWORD CALLBACK ProcessClient(LPVOID  _ptr)
{
	...
	while (1)
	{
		...
		case INITE_STATE:
			...
			break;
		case INTRO_STATE:
			ZeroMemory(Client_ptr->sendbuf, sizeof(Client_ptr->sendbuf));
			size = PackPacket(Client_ptr->sendbuf, INTRO, INTRO_MSG);
			retval = send(Client_ptr->sock, Client_ptr->sendbuf, size, 0);
			if (retval == SOCKET_ERROR)
			{
				err_display("Intro msg send()");
				Client_ptr->state = CONNECT_END_STATE;
				break;
			}
			Client_ptr->state = NICKNAME_SET_STATE;
			break;
		case NICKNAME_SET_STATE:
			...
			case CHATT_NICKNAME:
				...
				if (!NickNameSetting(Client_ptr))
				{
					size = PackPacket(Client_ptr->sendbuf, NICKNAME_EROR, NICKNAME_ERROR_MSG);
					retval = send(Client_ptr->sock, Client_ptr->sendbuf, size, 0);
					if (retval == SOCKET_ERROR)
					{
						err_display("Nickname Overlap msg send()");
						Client_ptr->state = CONNECT_END_STATE;
						break;
					}
					break;
		...
		case CHATT_INITE_STATE:
			...
			break;
		case CHATTING_STATE:
			ZeroMemory(Client_ptr->recvbuf, sizeof(Client_ptr->recvbuf));
			if (!PacketRecv(Client_ptr->sock, Client_ptr->recvbuf))
			{
				Client_ptr->state = CONNECT_END_STATE;
				break;
		...
		case CONNECT_END_STATE:
			RemoveNickName(Client_ptr->nickname);
...
bool NickNameSetting(_ClientInfo* _clientinfo)
{
	EnterCriticalSection(&cs);
	// 닉네임 초기설정 
	// 중복체크, 닉네임 리스트에 넣어주기, 클라이언트 닉네임 설정해주기, 

	char str[BUFSIZE];

	ZeroMemory(str, sizeof(str));
	UnPackPacket(_clientinfo->recvbuf, str);
	
	if (!NicknameCheck(str))
	{
		_clientinfo->state = NICKNAME_SET_STATE;
		LeaveCriticalSection(&cs);
		return false;
	}

	strcpy(_clientinfo->nickname, str);
...
{
	EnterCriticalSection(&cs);
	// 본인을 포함한 다른 유저에게 채팅내용 전송
	char msg[BUFSIZE];
	char input[BUFSIZE];
	ZeroMemory(msg, sizeof(msg));
	ZeroMemory(input, sizeof(input));

	UnPackPacket(_clientinfo->recvbuf, input);
	MaKeChattMessage(_clientinfo->nickname, input, msg);
	
	printf("%s\n", msg);

	ZeroMemory(_clientinfo->sendbuf, sizeof(_clientinfo->sendbuf));
	int size = PackPacket(_clientinfo->sendbuf, CHATT_MSG, msg);

	// 채팅 입장, 다른 플레이어에게 입장했다고 알림
	for (int i = 0; i < Client_Count; i++)
	{
		if (ClientInfo[i]->chatflag)
		{
			int retval = send(ClientInfo[i]->sock, _clientinfo->sendbuf, size, 0);
			if (retval == SOCKET_ERROR)
			{
				err_display("Chatting msg send()");

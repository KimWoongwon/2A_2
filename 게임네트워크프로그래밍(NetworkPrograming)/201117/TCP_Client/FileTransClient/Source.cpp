...
bool TransProcess(_ServerInfo* ptr)
{
	...
	int fsize = -1;
	int size_sum = ptr->file_info.nowsize;
	printf("전송중");
	while (size_sum < ptr->file_info.filesize)
	{
		//fseek(fp, ptr->file_info.nowsize + size_sum, SEEK_SET);

		ZeroMemory(msg, sizeof(msg));
		fsize = fread(msg, 1, SEND_DATA_SIZE, fp);	// bufsize만큼 나눠서 계속 보내다가
